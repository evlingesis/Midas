...
Простіше всього з прикладом.

**Що тут йдеться:** у кожному рецепті (Специфікації) є налаштування, яке каже Odoo **що робити, коли виробничник реально використав не рівно стільки сировини, скільки в рецепті**.

**Сценарій з життя:**

- Рецепт Полотна каже: на 1 шт треба `0,13 кг` Тканини. На 150 шт = `19,5 кг`.
- Реально виробничник розкроїв тканину і витратив `21 кг` (бо рулон не оптимально розкроївся). Тобто **перевитрата 1,5 кг**.
- Виробничник тисне «Виготовити все».

Що Odoo робить далі — залежить від налаштування рецепту. Є три варіанти:

- **«Дозволено» (`flexible`)** — Odoo нічого не питає. MO закривається мовчки. Перевитрата `1,5 кг` ніде не помічена, ніхто не дізнається.
- **«Дозволено з попередженням» (`warning`)** — Odoo показує діалог: «ти списав на 1,5 кг більше Тканини, ніж планували. Підтвердити?». Виробничник тисне «Так» — MO закривається. **Відхилення фіксується в історії MO**, його видно потім.
- **«Заблоковано» (`strict`)** — Odoo каже: «не можу закрити, є відхилення — виклич менеджера». Тільки користувач з правами менеджера може клацнути «Підтвердити».

**Що вибрано в Midas:** усі **50 рецептів на dev — у режимі `warning`**. Це не випадковість, а системне рішення — однакове налаштування на всіх рецептах.

**Чому саме `warning`, а не інші:**

- `flexible` був би **небезпечний**: виробничники могли б списувати тканину «у тиху» без жодного сліду. Через тиждень з'ясовується «у нас зник склад на 50 кг», а звідки — невідомо.
- `strict` був би **паралізуючий**: на кожну дрібну нормальну похибку (тканина не ріжеться ідеально рівно) треба дзвонити менеджеру і ждати. Виробництво стоїть.
- `warning` — **компроміс**: виробництво швидке (виробничник сам закриває MO), але всі відхилення лежать в історії. Якщо менеджер бачить що один рецепт постійно дає перевитрату — це сигнал переглянути рецепт.

**Що це означає для команди MidasPack:**

При кожному «Виготовити все» — якщо реально використав не рівно за планом — Odoo покаже діалог. **Не лякатись**, це нормально. Натиснути «Підтвердити» — MO закриється, перевитрата запишеться у його історію.

Якщо часто бачиш великі відхилення на одному й тому ж рецепті — повідом Оксану або Сергія, бо це означає що план у рецепті некоректний і його треба переглядати.
