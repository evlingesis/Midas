...
### 4.2. Як WIP облікується в Midas

Через те, що склад **1-step** (одна локація `WH/Stock`), готовий напівфабрикат **відразу опиняється в `WH/Stock`** — той самий загальний склад, де лежать сировина і готова продукція.

**Окремої «локації напівфабрикатів» не існує.**

**Що це означає на практиці:** після того як `01577` стане Done, у Quants товару Полотно з'являється `+150 шт у WH/Stock`. Воно фізично там же, де решта Полотен. Але одразу **«забронено»** для свого батька 01529 Дно — інше замовлення цю партію не отримає.

### 4.3. Як побачити «вільне» vs «зарезервоване»

Безпосередньо у списку Товарів — **не видно** прямо. Видно лише загальну кількість.

Але є два способи розібратись.
